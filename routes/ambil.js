...
const History = require('../model/histori')
...
router.post('/zona', async (req, res, next) => {
    const zona = new Zona({
        ...
        zona: [{
            ...
            nomor_zona: req.body.nomor_zona,
...
router.post('/getHistory', async (req, res) => {
    try {
        const  histori = await History.find({ mac: req.body.mac})
        if (histori.length == 0) {
            res.status(404).json({ status: "false",histori })
        } else {
            res.status(200).json({status:"true",histori})
        }
    } catch (err) {
        res.status(500).json({ status: err.status })
    }
...
router.post('/updateUser', async (req, res) => {
    let newValue = { $set: { latitude: req.body.latitude, longitude: req.body.longitude } }
    let query = { email: req.body.email }
    var data
    try {
        const updateUser = await userModel.find({ email : req.body.email})
        for ( i in updateUser){
             data = updateUser[i]
        }
        if (data.latitude | data.longitude == ""){
            userModel.updateOne(query, newValue, function(err, result) {
                if (err) throw err
                res.status(200).json({msg:"Berhasil",result})
            })
        } else {
            res.status(500).json({ message: "data sudah terisi"})
        }
    } catch (err) {
        res.status(400).json(err.message)
    }
})
router.post('/addhistori', async (req, res) => {
